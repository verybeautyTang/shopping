  ...
  <div>
          ...
          <el-table-column label="操作">
            <template slot-scope="scope">
              ...
              <el-tooltip content="角色分配" placement="top" :enterable ="false">
                <el-button @click="rightUser(scope.row)" type="warning" icon="el-icon-setting" size="mini"></el-button>
              </el-tooltip>
            </template>
          </el-table-column>
        </el-table>
    ...
    <el-dialog
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="closeRoles">确 定</el-button>
      </span>
  </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async closeRoles () {
      if (!this.SelectUid) {
        return this.$message.info('暂时没有角色要分配')
      }
      const { data: res } = await this.$http.put(`users/${this.userRoles.id}/role`, { rid: this.SelectUid })
      if (res.meta.status !== 200) {
        return this.$message.error('保存角色错误')
      } else {
