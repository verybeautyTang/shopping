<template>
  <div>
    ...
    <el-dialog title="分配权限"
      ...
      width="30%">
      <el-tree :data="roleList" :props="roleProp" ref="treeRef" show-checkbox default-expand-all :default-checked-keys="defkeys" node-key="id"></el-tree>
...
export default {
  ...
  methods: {
    ...
    // 点击确定分配权限
    async setRightSend () {
      const key = [...this.$refs.treeRef.getCheckedKeys(), ...this.$ref.treeRefs.getHalfCheckedKeys()]
      const idr = key.join(',')
      const { data: res } = await this.$http.post(`roles/${this.roleId}/rights`, { rids: idr })
      console.log(res)
