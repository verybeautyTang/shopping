         ...
         <el-table-column type="expand">
           <template slot-scope="scope">
             <el-row :class="['bdbottom',index === 0 ? 'bdtop' : '']" v-for="(item,index) in scope.row.children" :key="item.id">
               ...
               <el-col :span="18">
                 <!-- 通过for循环嵌套，渲染二级权限 -->
                 <el-row :class="[i === 0 ? '': 'bdtop']" v-for=" i in item.children" :key="i.id">
                   <el-col :span="6">
                     <el-tag closable type="success">{{i.authName}}</el-tag>
                     <i class="el-icon-caret-right"></i>
                   </el-col>
                   <el-col :span="17">
                     <el-row  :class="[ite === 0 ? '': 'bdtop']" v-for="ite in i.children" :key="ite.id">
                       <el-col>
                         <el-tag closable>{{ite.authName}}</el-tag>
                       </el-col>
                     </el-row>
                   </el-col>
                 </el-row>
